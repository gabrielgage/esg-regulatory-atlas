...
- Added direct term chips to the reusable `GlossaryHelpCard`.
- Added stable anchors to individual glossary term cards on `/glossary`.
- Wired contextual terms into assessment, regulations, Data Quality, timeline, briefing, compare, markets, sectors, jurisdiction and value-chain help cards.
- Added data guardrail coverage so route-level `termIds` must resolve to known glossary IDs.
- Added smoke coverage for direct glossary term links.

## Phase 1AZ Product Rationale

The simplification roadmap identified glossary term linking as a lower-cognitive-load follow-up after the public journey was calmer. Phase 1AZ keeps the glossary as a support layer, not a new heavy page section: users can jump from label-heavy assessment, timeline, regulation, market, sector and value-chain surfaces to the exact plain-language term they need while preserving the caveat that glossary content is orientation only.
...
- Added the sample scan to `/advisory`, covering priority records, facts to confirm, evidence package, first 30-day actions, source-review notes and caveats.
- Improved dark-mode styling across the advisory proof point and service cards.
- Added smoke coverage for the advisory sample output.

## Phase 1AX Product Rationale

The simplification roadmap called for static advisory sample outputs after the core journey was calmer. Phase 1AX makes the Advisory Atlas offer more concrete without adding SaaS infrastructure: users can see what a manual exposure scan might return, copy the structure, and still see that the output is source-linked orientation rather than legal advice, official verification or a definitive applicability determination.

## Phase 1AW Changes Delivered

- Updated dataset metadata and changelog to `0.5.67 - May 2026`.
- Added `data/routeRegistry.ts` to classify each route by placement, visibility, robots, navigation group, template status and user decision.
- Updated `components/Header.tsx` so primary navigation and the More menu are generated from the route registry.
- Added data guardrail tests for static route coverage, internal/noindex launch routing and contextual dynamic templates.
- Updated agent, roadmap, release and Notion context so future routes are classified before being linked.

## Phase 1AW Product Rationale

The simplification roadmap called out route metadata classification because the Atlas had accumulated many useful surfaces, and manual header edits made it too easy for future pages to crowd the public journey. Phase 1AW gives the product a simple route-governance layer: Start, Assessment, Markets, Regulations and Advisory remain primary; planning, trust and commercial previews remain grouped; dynamic details stay contextual; `/launch` stays internal/noindex.

## Phase 1AV Changes Delivered

- Updated dataset metadata and changelog to `0.5.66 - May 2026`.
- Added `data/legalNotices.ts` as the shared source for recurring legal, copied-output, manual-request, commercial-preview and seed-record caveats.
- Added `components/LegalNotice.tsx` for consistent legal notice styling in light and dark mode.
...
- Added a data guardrail test to preserve the core no-legal-advice, source-validation and no-live-infrastructure caveats.

## Phase 1AV Product Rationale

The simplification roadmap called out shared disclaimer layers as a legal-design improvement after the public journey was made calmer. Phase 1AV reduces future drift: new routes can reuse the same caveat source and visual component rather than adding one-off legal text that may conflict with the site-wide no-legal-advice, no-production-alerts and source-validation boundaries.
...
- Phase 1AZ validation: `npm run lint` passed locally.
- Phase 1AZ validation: `npm run check:data` passed locally.
- Phase 1AZ validation: `npm run build` passed locally with the webpack build path and generated 389 static pages.
- Phase 1AZ validation: `git diff --check` passed.
- Phase 1AZ targeted local browser smoke (`npm run test:e2e -- tests/glossary.spec.ts tests/glossary-help.spec.ts`) was blocked by sandbox host-binding permissions: `listen EPERM: operation not permitted 127.0.0.1:3000`. This matches the existing local server limitation; CI remains the authoritative browser-smoke signal.
- Phase 1AZ out-of-scope scan found only guardrail/documentation/caveat/test mentions of Stripe, Supabase, Mapbox, checkout, billing, webhooks, scraping, cron and related future infrastructure; no implementation code or dependencies were added.
...
- Phase 1AX validation: `git diff --check` passed.
- Phase 1AX local browser smoke: the Playwright server host-binding issue was fixed by binding `next start` to `127.0.0.1`; the local run then stopped on the known missing Chromium headless-shell cache. CI should provide the authoritative browser-smoke signal.
- Phase 1AX out-of-scope scan found only guardrail/documentation/caveat mentions of Stripe, Supabase, Mapbox, checkout, billing, webhooks, scraping, cron and related future infrastructure; no implementation code or dependencies were added.
- Phase 1AW validation: `npm run lint` passed locally.
- Phase 1AW validation: `npm run check:data` passed locally.
- Phase 1AW validation: `npm run build` passed locally with the webpack build path and generated 389 static pages.
- Phase 1AW validation: `git diff --check` passed.
- Phase 1AW out-of-scope scan found only guardrail/documentation/caveat mentions of Stripe, Supabase, Mapbox, checkout, billing, webhooks, scraping, cron and related future infrastructure; no implementation code or dependencies were added.
- Phase 1AV validation: `npm run lint` passed locally.
- Phase 1AV validation: `npm run check:data` passed locally.
- Phase 1AV validation: `npm run build` passed locally with the webpack build path and generated 389 static pages.
- Phase 1AV validation: `git diff --check` passed.
- Phase 1AV out-of-scope scan found only guardrail/documentation/caveat mentions of Stripe, Supabase, Mapbox, checkout, billing, webhooks, scraping, cron and related future infrastructure; no implementation code or dependencies were added.
