...
| P2 | Glossary term linking | Advanced in `0.5.70`: contextual help cards now expose direct glossary term links and the glossary page has stable term anchors. |
...
- Contextual glossary links should use `components/GlossaryHelpCard.tsx` with valid IDs from `data/glossary.ts`; avoid one-off glossary chip rows that can drift from the shared term set.
