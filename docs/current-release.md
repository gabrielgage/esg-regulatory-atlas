...
### 0.5.69 - Print Output Polish

- Replaced the hardcoded global print header with `data-print-title` and `data-print-subtitle` metadata sourced from `DATASET_META`.
- Added A4 print margins, cleaner printed card borders, page-break helper classes and source URL expansion for external links.
- Updated the reusable print button with print hiding and an explicit accessible label.
- Added guardrail coverage to prevent stale hardcoded print editions from returning.
- Documented the stale-print-edition bug and prevention rule in the issue log.

### 0.5.68 - Advisory Sample Output

- Added `data/advisorySampleOutputs.ts` with a structured sample supplier/exporter exposure scan.
- Added `components/AdvisorySampleOutput.tsx` with priority questions, priority records, facts to confirm, evidence package, first actions and source-review notes.
- Added a copyable sample scan on `/advisory` so buyers can see what a manual advisory output might contain.
- Improved dark-mode styling across the advisory proof-point and service cards.
- Kept the sample static, caveated and source-review oriented; it does not add accounts, payments, automation, legal advice or definitive applicability.

### 0.5.67 - Route Metadata Classification

- Added `data/routeRegistry.ts` as the single source for route placement, visibility, robots, group and user-decision metadata.
- Updated `components/Header.tsx` so primary navigation and the More menu are sourced from the route registry.
- Added data guardrails so static routes are represented in the registry, `/launch` stays internal/noindex and dynamic templates stay contextual.
- Kept the public header focused on Start, Assessment, Markets, Regulations and Advisory.
- Preserved current route behavior; this is navigation governance, not new infrastructure or regulatory content.

### 0.5.66 - Shared Legal Notice Components

- Added `data/legalNotices.ts` as the shared source for recurring legal, copy-output, commercial-preview, manual-request and regulation-record caveats.
- Added `components/LegalNotice.tsx` so disclaimer, footer and request surfaces share consistent light/dark mode styling.
- Updated the top disclaimer banner, footer disclaimer, copied-output note and manual request panel to use the shared notice system.
- Added a data guardrail test so canonical legal notices keep the core no-legal-advice, source-validation and no-live-infrastructure caveats.
- Kept route-specific localized disclaimers and record-specific caveats intact where those surfaces need their own wording.

### 0.5.65 - Plans Page Simplification

- Reworked `/plans` around two current choices: use the free Atlas or request a manual advisory-supported scan.
- Moved Premium Intelligence and Enterprise/API Future into secondary validation cards with clearer caveats.
- Collapsed the full free/premium/advisory/enterprise comparison table behind a details disclosure.
- Removed the visible manual validation-loop card grid from the first-load page so commercial planning context no longer competes with user choices.
- Preserved manual request guidance and no-infrastructure caveats.

### 0.5.64 - Value-Chain Lane Simplification

- Reworked `/value-chain` around six primary exposure lanes: suppliers, trade/imports, products/claims, portfolio/finance, own operations/governance and customer pressure.
- Added lane-specific start questions, evidence prompts, first actions and suggested owner functions.
- Reduced repeated chip walls by showing only essential topic, review and exposure-count context on each lane.
- Added copyable lane summaries with source-review context and legal caveats.
- Kept lane outputs framed as seed intelligence for evidence planning, not complete value-chain legal coverage or entity-specific applicability determinations.

### 0.5.63 - Interface Language Clarity

- Updated the language selector label from generic language to interface language.
- Added a localized tooltip caveat explaining that language selection changes interface guidance only.
- Clarified that regulatory records are not official legal translations.
- Improved dark-mode styling for the language control.
- Kept existing English, Spanish, Dutch, French, German and Portuguese interface chrome support intact.

### 0.5.62 - Sector Finder Simplification

- Reworked `/sectors` from aggregate metrics and dense cards into a searchable sector finder.
- Added sector family filters for capital markets, industrial/infrastructure, consumer/supply-chain and public/digital contexts.
- Shows practical trigger summaries, review-first records, source-count context and review cues inside calmer sector cards.
- Added an empty state and assessment handoff so users can move from sector browsing into entity-specific profile triage.
- Kept sector counts caveated as current seed coverage, not complete sector legal inventories or applicability determinations.

### 0.5.61 - Reusable Advisory Scan CTA

- Added `components/AdvisoryScanCTA.tsx` as the shared manual advisory-scan request path.
- Kept `components/MarketBriefingCTA.tsx` as a stable wrapper so market, sector, briefing and jurisdiction pages preserve existing behavior while sharing copy and caveats.
- Updated assessment, regulation detail page and regulation drawer CTAs to use the shared advisory-scan component.
- Standardized the caveat that advisory scans are manual, source-linked planning outputs and are not legal opinions, official source verification or definitive applicability determinations.
- Kept all conversion paths mailto-only with no accounts, payments, database, automation or production email backend.
