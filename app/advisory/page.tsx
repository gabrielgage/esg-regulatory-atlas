...
import { AdvisorySampleOutput } from "@/components/AdvisorySampleOutput";
  ...
  return (
    <main id="main-content" className="min-h-screen pb-12">
      ...
      <div className="mx-auto max-w-7xl space-y-5 px-4 py-5 md:px-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-950/70">
          <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
            <div>
              <div className="flex items-center gap-2 text-sm font-semibold text-teal dark:text-mint">
                <BriefcaseBusiness className="h-4 w-4" />
                Fastest monetization path
              </div>
              <h1 className="mt-3 text-3xl font-bold tracking-tight text-ink dark:text-white">Turn the free Atlas into client-ready advisory work</h1>
        ...
        <AdvisorySampleOutput sample={advisorySampleOutputs[0]} />

        <section className="grid gap-4 lg:grid-cols-2">
          {advisoryServices.map((service) => (
            <article key={service.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-950/70">
              <div className="flex flex-wrap gap-2">
                {service.bestFor.slice(0, 3).map((persona) => (
                  <Badge key={persona} className="border-slate-200 bg-slate-50 text-slate-600 dark:border-white/10 dark:bg-white/5 dark:text-slate-300">
                    {persona}
                  </Badge>
                ))}
              </div>
              <h2 className="mt-4 text-xl font-bold tracking-tight text-ink dark:text-white">{service.name}</h2>
              ...
              <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900 dark:border-amber-400/30 dark:bg-amber-400/10 dark:text-amber-100">{service.caveat}</p>
              <a href={service.ctaHref} className="mt-4 inline-flex items-center justify-center gap-2 rounded-xl bg-ink px-4 py-3 text-sm font-semibold text-white hover:bg-slate-800 dark:bg-mint dark:text-ink dark:hover:bg-teal">
                {service.ctaLabel}
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-950/70">
          <h2 className="text-lg font-semibold text-ink dark:text-white">What an advisory scan should answer</h2>
...
function Metric({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
      ...
      <div className="mt-2 font-semibold text-ink dark:text-white">{value}</div>
    </div>
  );
}
...
function Question({ text }: { text: string }) {
  return <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm font-semibold leading-6 text-ink dark:border-white/10 dark:bg-white/5 dark:text-white">{text}</div>;
}
