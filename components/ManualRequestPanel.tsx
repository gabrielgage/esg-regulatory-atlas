  ...
  return (
    <section className="rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-teal">Manual request path</p>
          <h2 className="mt-1 text-xl font-semibold text-ink">{title}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{body}</p>
          <p className="mt-3 text-xs leading-5 text-slate-500">{timing}</p>
        </div>
        <a href={href} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-ink px-4 py-3 text-sm font-semibold text-white hover:bg-slate-800">
      ...
      <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600">
        {values.map((value) => (
          <li key={value} className="flex gap-2">
            <Send className="mt-1 h-3.5 w-3.5 shrink-0 text-teal" />
            <span>{value}</span>
          </li>
        ))}
      </ul>
